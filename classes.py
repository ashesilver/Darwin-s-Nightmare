...
text_4 = ["si j'étais vous", "j'éviterais de m'aventurer...","...dans la forêt.", "quiconque y est rentré...","...n'est jamais revenu."]
text_5 = ["quel beau temps !"]
text_6 = ["déchainé mooooi !!!"]
text_7 = ["la", "baignoire", "est", "dans le sac", "je répète,", "la baignoire...","est dans le sac !!"]
text_8 = ["ma mère ma raconté"," une histoire un jour", "la légende d'un héro...", "qui se serait aventuré"," dans un lieu...", "au sud ouest d'ici...","mais je n'ai jamais"," oser y mettre les pieds."]
text_9 = ["la légende de cette île raconte...", "que si on dérobe"," les 3 idoles sacrées,...", "un ancien s'abatrat sur l'île,...", "à partir des ruines"," situer au nord est...", "si le mal est combattu,", "l'île sera libéré de son fardeau"]
text_10 = ["est ce que tu aime manger ?", "moi j'adore ça !", "d'ailleurs !", "10 kiwis pour 2 !","au lieu de 3.50 !", "affaire à saisir !", "...", "aah ça me jambe la gratte !"]
text_11 = ["on s'est fait berner !"," par les coquelicots !!!"]
text_12 = ["un jour,...","j'ai sauté dans un trou","comme celui ci !", "depuis, je suis invincible!!!"]
...
def store_all_data(pos, lives, shield, inventory):
...
def load_all_data():
    ...
    return x, y, lives, shield, inventory
...
class Player(swapcore.kernel.Area):
    ...
    def lives(self, value):
        self._lives = value
        if self.lives <= 0:
            RUNNING = gameover_screen(screen, self)
            if RUNNING:
                self.lives = 3
            else:
                pygame.quit()
...
class ZombieSpawn:
    ...
    def update(self):
        if time.time() - self.last_spawn >= self.next_spawn:
